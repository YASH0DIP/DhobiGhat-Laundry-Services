...
try {
    ...
    if (count($results) <= 0) {
        header('Location: ../transaction.php?type=error&message=You Have No Order Yet!');
        exit();
    }
    ...
    header('Location: ../transaction.php?type=success&message=Order Cancelled Successfully!');

   

} catch (\Throwable $th) {
    generate_logs('Transaction Cancel', $th);
    header('Location: ../transaction.php?type=error&message=Something Went Wsrong!');
}
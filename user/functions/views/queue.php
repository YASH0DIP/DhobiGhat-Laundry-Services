...
$user_id = $_SESSION['id'];
$sql = "SELECT l.status,l.comments ,l.id AS laundry_id, l.kilo, p.price, p.name, p.unit, t.id, t.customer_id, t.total, l.created_at, c.fullname, ROW_NUMBER() OVER (ORDER BY status DESC, kilo ASC) AS queue_number 
        FROM laundry AS l
        JOIN prices AS p ON l.type = p.id
        JOIN transactions AS t ON l.transaction_id = t.id
        JOIN customers AS c ON t.customer_id = c.id
        WHERE l.status >= 0 AND l.status < 4 AND l.customer_id = $user_id";
    ...
    ?>
        <tr>
            ...
            <?php if ($row['status'] < 5): ?>
                ...
                <a class="mx-1 text-decoration-none" style="color: #858796; font-weight: 600;" href="tracking.php?id=<?php echo $row['id']; ?>" target="_blank"><i class="far fa-credit-card" style="font-size: 20px;"></i> Tracking</a>
                <a class="mx-1 text-decoration-none" style="color: #858796; font-weight: 600;" href="invoice.php?id=<?php echo $row['id']; ?>" target="_blank"><i class="fas fa-print" style="font-size: 20px;"></i> Invoice</a>
            <?php else: ?>
